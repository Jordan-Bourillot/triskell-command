...
"""
    ...
    # démarchés surtout par réseaux sociaux. S'appuie sur la MÊME table
    # `prospects` qu'Obélisk : on réutilise list_creators / update_creator
    # / get_creator / delete_creator, plus le canal de contact, la date de
    # prochaine relance et le lien de démo (colonnes migration SQL 51).
    # ==================================================================
    # Canaux de contact reconnus (étiquette FR côté UI). Texte libre en
    # base, mais on normalise à la saisie pour que les filtres marchent.
    _CREATOR_CHANNELS = ("instagram", "tiktok", "youtube", "facebook",
                         "email", "autre")
    # Au-delà de ce délai sans relance programmée, un créateur « contacté »
    # est considéré « à relancer » dans la vue dédiée. Réglable au besoin.
