...
const Revenue = {
    ...
    if (!data || !data.ok) {
      slot.innerHTML = `
        <div class="card p-10 text-center">
          ...
          <p class="text-text-muted">${(data && data.error) || 'Connecte-toi à Supabase dans les Réglages.'}</p>
        </div>
      `;
      return;
    }
  ...
  _renderHero(d) {
    ...
    const evolPct = (prev.total_cents > 0)
      ? Math.round(((cur.total_cents - prev.total_cents) / prev.total_cents) * 100)
      : 0;
    const evolClass = evolPct > 0 ? 'text-success' : evolPct < 0 ? 'text-danger' : 'text-text-muted';
    const evolIcon = evolPct > 0 ? '↑' : evolPct < 0 ? '↓' : '→';
    return `
      <div class="card-hero p-6 sm:p-10" data-accent="${evolPct >= 0 ? 'success' : 'warning'}">
        <div class="hero-kicker text-success mb-2">${(cur.label || 'Mois en cours').toUpperCase()}</div>
        <div class="flex items-baseline gap-4 flex-wrap">
          <div class="font-display text-3xl sm:text-5xl font-bold">${this._fmtEuro(cur.total_cents)}</div>
          <div class="text-base ${evolClass} font-semibold">${evolIcon} ${Math.abs(evolPct)}% vs ${prev.label || 'mois précédent'}</div>
        </div>
        <p class="text-text-secondary text-sm mt-3 max-w-xl">
          ${cur.transactions_count || 0} transactions ce mois — ${this._fmtEuro(cur.total_cents - (prev.total_cents || 0))} d'évolution.
        </p>
      </div>
    `;
  },
        ...
        <div class="card p-6 flex items-start gap-4">
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-text-muted">
              Basée sur ${fc.pipeline_count || 0} intakes en pipeline × taux de conversion observé (${fc.conversion_rate_pct || 0}%) + encaissements déjà reçus.
  ...
  _preview() {
    return `<div class="card p-10 text-center text-text-muted">Mode preview.</div>`;
  },

  _stat({label, value, delta}) {
    return `
      <div class="card p-5">
        <div class="text-[10px] font-bold tracking-widest text-text-muted mb-2">${label.toUpperCase()}</div>
