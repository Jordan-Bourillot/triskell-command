"""DAO Supabase pour les intakes Studio WoW.
...
Tables :
  - wow_intakes : voir studio-wow/db/01_init.sql
...
def _load_service_config() -> Optional[dict]:
    ...
    if settings_path.exists():
        try:
            ...
            logger.debug("wow._load_service_config: %s", exc)
    return None
...
def _service_sb():
    ...
    try:
        ...
        logger.warning("wow._service_sb: %s", exc)
        return None
    return _SERVICE_CLIENT
...
def list_intakes(*, status: Optional[str] = None, limit: int = 50) -> list[dict]:
    ...
    if sb is None:
        return []
    try:
        q = sb.table("wow_intakes").select("*").order("created_at", desc=True).limit(limit)
        if status:
            q = q.eq("status", status)
        return q.execute().data or []
    except Exception as exc:
        logger.warning("wow.list_intakes: %s", exc)
        return []
...
def get_intake(intake_id: str) -> Optional[dict]:
    ...
    if sb is None:
        return None
    try:
        rows = (sb.table("wow_intakes").select("*")
                .eq("id", intake_id).limit(1).execute().data)
        return rows[0] if rows else None
    except Exception as exc:
        logger.warning("wow.get_intake: %s", exc)
        return None
...
def update_intake_status(intake_id: str, new_status: str, *, error_message: str = "") -> bool:
    ...
    try:
        ...
        logger.warning("wow.update_intake_status: %s", exc)
        return False
...
def dispatch_now(intake_id: str) -> tuple[bool, str]:
    ...
    """
    import requests
    url = "https://studio-wow.fr/.netlify/functions/dispatch-site-build"
    try:
