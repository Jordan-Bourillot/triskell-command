"""Registre des clients Porte-Voix suivis.

La source unique est la collection « clients » du stockage Porte-Voix
(base partagée, fichier local en secours — voir stockage.py) : aucune
donnée de client en dur dans le code. Chaque fiche porte l'entreprise,
le métier, la ou les villes, les concurrents désignés (3 au maximum :
ceux que le client veut voir dans son rapport), les questions
personnalisées éventuelles (sinon les formulations d'audit du métier
servent), le palier d'abonnement et la date d'entrée.

Un client désactivé (résiliation, pause) reste dans le registre — son
...
from . import audit, moteur, stockage
...
def _charger() -> list:
    entrees = stockage.lire("clients", [])
    return entrees if isinstance(entrees, list) else []


def _sauver(entrees: list) -> None:
    if not stockage.ecrire("clients", entrees):
        raise ValueError("enregistrement impossible pour l'instant (base "
                         "partagée injoignable) — réessaie dans un moment")
...
def _date_entree(texte: str) -> str:
    """La date d'entrée, toujours en AAAA-MM-JJ (le robot mensuel compare
    ce champ en texte : un format libre fausserait sa décision)."""
    ...
    try:
        ...
        raise ValueError(f"date d'entrée invalide : « {texte} » "
                         f"(attendu : AAAA-MM-JJ, ex. 2026-07-09)")
        ...
        for e in entrees:
            if e.get("actif", True) and moteur.correspond(
                ...
                raise ValueError(
                    ...
                    f"{e.get('entreprise')} (id {e.get('id')})")
...
def modifier_client(ident: str, **champs) -> dict:
    ...
    with stockage.VERROU:
        ...
        if "fait" in propres or "avenir" in propres:
            # Horodate la mise à jour des rubriques : l'écran et le robot
            # savent ainsi repérer des rubriques restées telles quelles
            # depuis le dernier rapport (règle de rétention).
            propres["rubriques_maj"] = datetime.now(timezone.utc).isoformat()
